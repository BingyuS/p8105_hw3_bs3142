...
tidy_noaa %>%
  filter(!is.na(snow)) %>%
  count(snow) %>%
  filter(min_rank(desc(n)) < 4) #get the most commonly observed value
```

For snowfall, the most commonly observed value was **0mm**, meaning snow days are less frequent compared to non-snow days. For days that did snow, the most commonly observed value was **25mm**, followed by 13mm as the second most frequent value.
